...
import pdfplumber, io, os, pymysql
...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],  # En producción, restringir a pro.w24h.es
...
async def upload_pdf(file: UploadFile = File(...), db = Depends(get_db)):
    ...
    try:
        ...
        cur.execute("INSERT INTO montage (event_id, area, details, table_map) VALUES (%s, %s, %s, %s)",
                    (event_id, data["montage"]["area"], data["montage"]["details"], io.JSONEncoder().encode(data["montage"]["table_map"])))
...
def export_excel(event_id: int, dept: str, db = Depends(get_db)):
    ...
    if res.get("montage"): 
        df = pd.concat([df, pd.DataFrame([{"item_name":"📋 MONTAJE", "notes": res["montage"]}])], ignore_index=True)
